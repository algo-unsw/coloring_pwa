...
#var run_b = w1 + wk6 * u + wk5 * u + wk4 * u + wk3 * u+ wk2 * u  + w_s * b_s+ c * u; #branching algo when width based algo is not triggered
var run_b = w1 + (sum{i in 2..N} wk[i]*(u-alpha[i])) + w_s * b_s+ c * u; #branching algo when width based algo is not triggered
# For i=1..6: |C| - gamma_i*|S|<=u-alpha_i

var run_w = 4^alpha[1]; #width based algo
var run;
